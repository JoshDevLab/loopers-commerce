...
public class BrandV1Controller {
    ...
    @GetMapping("/{id}")
    public ApiResponse<BrandDetailResponse> getBrandDetail(@PathVariable Long id) {
        BrandInfo brandInfo = brandFacade.getBrandByIdWithProducts(id);
        return ApiResponse.success(BrandDetailResponse.from(brandInfo));
    }
}
